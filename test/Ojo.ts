...
describe("Deploy", function () {
    ...
    it("relayer access control", async  function(){
        const {oracle, otherAdmin, relayer, otherRelayer, relayerRole} = await loadFixture(deployOracle);

        // check relayer role was assigned to relayer
        expect(await oracle.hasRole(relayerRole, relayer.address)).eq(true)

        // grant role with non admin
        expect(oracle.connect(otherAdmin).grantRole(relayerRole, otherRelayer.address)).to.be.revertedWithCustomError;

        // grant role with admin
        await oracle.grantRole(relayerRole, otherRelayer.address)
        expect(await oracle.hasRole(relayerRole, otherRelayer.address)).eq(true)

        // revoke role with non admin
        expect(oracle.connect(otherAdmin).revokeRole(relayerRole, otherRelayer.address)).to.be.revertedWithCustomError;

        // revoke role with admin
        await oracle.revokeRole(relayerRole, otherRelayer.address)
        expect(await oracle.hasRole(relayerRole, otherRelayer.address)).eq(false)
    })
    ...
    it("post price, deviation and median data",async () => {
        ...
        let resolveTime = (blockTimestamp+1000).toString()
    ...
    it("whitelist list check",async () => {
        const{oracle, otherAdmin,relayer, otherRelayer} = await loadFixture(deployOracle);
        ...
        let prices=await oracle.connect(otherAdmin).getPriceDataBulk(data.map(d=>d.assetName))
        expect(prices.length).eq(data.length)
    })

    it("median status check",async()=>{
        const{oracle, relayer} = await loadFixture(deployOracle);
    ...
    it("price relay resolve time check",async()=>{
        ...
        await time.increaseTo(blockTimestamp+1000)
